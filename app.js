...
const state={page:"admin",tournament:null,adminDraft:null,generatedLink:""};
...
function save(){if(state.tournament)localStorage.setItem(KEY,JSON.stringify(state.tournament))}
...
function makeBoardLink(t){return window.location.href.split("#")[0]+"#board="+encodeURIComponent(encodeBoard(t))}
...
function header(t){return `<header class="topbar"><div><div class="brand">SHUTTLE <span>SYNDICATE</span></div><div class="subbrand">${esc(t.type).toUpperCase()} · ${APP_VERSION}</div></div><button class="admin-link" id="adminHome">ADMIN</button></header>`}
...
function courtCard(t,g){return `<div class="court"><div class="court-head"><div><span class="live-dot"></span><b>COURT ${g.court}</b></div><span class="small">NOW · ${g.id}</span></div><div class="pair">${g.a.map(id=>esc(playerMap(t)[id]?.name||id)).join(" + ")}</div><div class="vs">VS</div><div class="pair">${g.b.map(id=>esc(playerMap(t)[id]?.name||id)).join(" + ")}</div><div class="score-row"><div class="score-box sky"><div class="score-label">SIDE A</div><input inputmode="numeric" pattern="[0-9]*" value="" id="s1-${g.id}"></div><div class="vs">—</div><div class="score-box net"><div class="score-label">SIDE B</div><input inputmode="numeric" pattern="[0-9]*" value="" id="s2-${g.id}"></div></div><button class="btn good wide" onclick="submitScore('${g.id}')">CONFIRM SCORE</button></div>`}
...
function scheduleRow(t,g){const result=g.score?`${g.score[0]} — ${g.score[1]}`:"";return `<div class="game-row ${g.status}"><div><b>${g.id}</b><div class="small muted">${g.court?"C"+g.court:"—"}</div></div><div class="fixture">${fixture(g,t)}${result?`<div class="score-result">${result}</div>`:""}</div><span class="status ${g.status}">${g.status==="done"?"COMPLETED":g.status==="live"?"LIVE":"UPCOMING"}</span>${g.status==="done"?`<button class="btn tiny" onclick="editLeagueScore('${g.id}')">EDIT SCORE</button>`:""}</div>`}
...
function playoffMatchCard(t,m,key){const p=playoffState(t);if(!m)return "";const score=m.score;return `<div class="match-card ${m.winner!==undefined?"complete":""}"><div class="match-title">${esc(m.label)}</div><div class="team-line"><span>${esc(m.sideA||"Waiting")}</span><b>${score?score[0]:"—"}</b></div><div class="team-line"><span>${esc(m.sideB||"Waiting")}</span><b>${score?score[1]:"—"}</b></div>${m.sideA&&m.sideB&&!score?`<div class="playoff-score"><input class="field" id="po-a-${key}" inputmode="numeric" placeholder="Score"><input class="field" id="po-b-${key}" inputmode="numeric" placeholder="Score"><button class="btn good wide" onclick="submitPlayoffScore('${key}')">CONFIRM SCORE</button></div>`:""}${m.winner!==undefined?`<div class="winner-line">WINNER · ${esc(m.winner)}</div>`:""}</div>`}
...
load();const hash=window.location.hash;if(hash==="#admin"){state.page="admin"}else{const boardHash=hash.match(/^#board=(.+)$/);if(boardHash){const shared=decodeBoard(decodeURIComponent(boardHash[1]));if(shared){state.tournament=shared;state.page="teams";save()}}if(state.tournament)state.page="teams"}render();
