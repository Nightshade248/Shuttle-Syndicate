...
function makeBoardLink(t){return window.location.href.split("#")[0]+"#board="+encodeURIComponent(t.id)}
...
load();const hash=window.location.hash;if(hash==="#admin"){state.page="admin";state.shared=false;state.canScore=true;render()}else{const boardHash=hash.match(/^#board=(.+)$/);if(boardHash){const id=decodeURIComponent(boardHash[1]);state.shared=true;state.canScore=false;state.page="teams";render();connectSharedTournament(id)}else if(state.tournament){state.shared=false;state.canScore=true;state.page="teams";render()}else{state.page="admin";state.shared=false;state.canScore=true;render()}}
