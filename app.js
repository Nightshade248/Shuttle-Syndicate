...
async function publishTournament(t){const client=await initRealtimeClient();if(!client||!t?.id)return false;const fn=state.shared?"upsert_shuttle_player_state":"upsert_shuttle_tournament";const args=state.shared?{p_id:t.id,p_state:t}:{p_id:t.id,p_admin_key:state.adminKey,p_state:t};if(!state.shared&&!state.adminKey)return false;const {data,error}=await client.rpc(fn,args);if(error){console.warn("Realtime publish failed",error);return false}return !!data}
...
function makeBoardLink(t){const url=new URL(PUBLIC_BOARD_BASE);url.searchParams.set("board",t.id);return url.toString()}
...
function nav(){return `<nav class="nav"><button data-page="teams" class="${state.page==="teams"?"active":""}"><span class="ico">◈</span>Teams</button><button data-page="live" class="${state.page==="live"?"active":""}"><span class="ico">●</span>Live</button><button data-page="schedule" class="${state.page==="schedule"?"active":""}"><span class="ico">☷</span>Schedule / Ranking</button><button data-page="playoffs" class="${state.page==="playoffs"?"active":""}"><span class="ico">♛</span>Playoffs</button></nav>`}
function header(t){return `<header class="topbar"><div><div class="brand">SHUTTLE <span>SYNDICATE</span></div><div class="subbrand">${esc(t.type).toUpperCase()} · ${APP_VERSION}</div></div>${state.shared?"":`<button class="admin-link" id="adminHome">ADMIN</button>`}</header>`}
...
function livePage(t){
 ...
 const next=upcoming.length?upcoming.map((g,i)=>`<div class="next-item ${i===0?"best":""}"><div class="next-label">${i<t.courts?"NEXT":"CANDIDATE"} · ${g.id}</div><div class="fixture">${fixture(g,t)}</div><div class="small muted">Availability · rest · partner/opponent history · remaining games</div></div>`).join(""):'<div class="muted">No eligible next game. Waiting for available players.</div>';
...
function schedulePage(t){const selected=state.playerFilter||"ALL",gs=t.games.filter(g=>selected==="ALL"||[...g.a,...g.b].includes(selected)),r=rankings(t);return `<section class="hero page-hero"><div class="eyebrow">SCHEDULE / RANKING</div><h1>SCHEDULE / RANKING</h1><div class="filter"><select class="field" id="playerFilter"><option value="ALL" ${selected==="ALL"?"selected":""}>ALL — ENTIRE SCHEDULE</option>${t.players.map(p=>`<option value="${p.id}" ${selected===p.id?"selected":""}>${esc(p.name)}</option>`).join("")}</select><span class="small muted">Live court order may change dynamically.</span></div></section><section class="card compact-section"><div class="section-head"><h2>${selected==="ALL"?"Entire Schedule":esc(playerMap(t)[selected]?.name||"Player")+"'s Fixtures"}</h2><span class="small muted">${gs.length} fixture(s)</span></div><div class="schedule-list">${gs.map(g=>scheduleRow(t,g)).join("")}</div></section>${rankingBoard(t,r)}`}
function scheduleRow(t,g){const result=g.score?`${g.score[0]} — ${g.score[1]}`:"";return `<div class="game-row ${g.status}"><div><b>${g.id}</b><div class="small muted">${g.court?"C"+g.court:"—"}</div></div><div class="fixture">${fixture(g,t)}${result?`<div class="score-result">${result}</div>`:""}</div><span class="status ${g.status}">${g.status==="done"?"COMPLETED":g.status==="live"?"LIVE":"UPCOMING"}</span>${state.canScore&&g.status==="done"?`<button class="btn tiny" onclick="editLeagueScore('${g.id}')">EDIT SCORE</button>`:""}</div>`}
window.editLeagueScore=id=>{const t=state.tournament,g=t.games.find(x=>x.id===id);if(!g?.score)return;const a=prompt(`Edit ${id} — Side A score`,g.score[0]);if(a===null)return;const b=prompt(`Edit ${id} — Side B score`,g.score[1]);if(b===null)return;const s1=num(a,-1),s2=num(b,-1);if(s1<0||s2<0||s1===s2){alert("Enter two different non-negative scores.");return}g.score=[s1,s2];g.winner=s1>s2?0:1;t.playoff={};save();render()};
...
function teamRank(t,title,cls,arr){const ranked=arr.map((x,i)=>({...x,rank:i+1}));const top=t.playerCount>=16?6:4;return `<div class="card ${cls==="sky"?"team-sky":"team-net"}"><div class="team-title ${cls}">${title}</div><div class="small muted">${top===6?"#1–#2 direct semifinals · #3–#6 qualifier path":"TOP 4 PLAYOFF QUALIFIERS"}</div>${rankTable(ranked,top,4)}</div>`}
...
function playoffMatchCard(t,m,key){const p=playoffState(t);if(!m)return "";const score=m.score;return `<div class="match-card ${m.winner!==undefined?"complete":""}"><div class="match-title">${esc(m.label)}</div><div class="team-line"><span>${esc(m.sideA||"Waiting")}</span><b>${score?score[0]:"—"}</b></div><div class="team-line"><span>${esc(m.sideB||"Waiting")}</span><b>${score?score[1]:"—"}</b></div>${state.canScore&&m.sideA&&m.sideB&&!score?`<div class="playoff-score"><input class="field" id="po-a-${key}" inputmode="numeric" placeholder="Score"><input class="field" id="po-b-${key}" inputmode="numeric" placeholder="Score"><button class="btn good wide" onclick="submitPlayoffScore('${key}')">CONFIRM SCORE</button></div>`:""}${m.winner!==undefined?`<div class="winner-line">WINNER · ${esc(m.winner)}</div>`:""}</div>`}
